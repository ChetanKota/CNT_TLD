...
void TLD::processFrame(const cv::Mat& img1,const cv::Mat& img2,vector<Point2f>& points1,vector<Point2f>& points2,BoundingBox& bbnext,bool& lastboxfound, bool tl, FILE* bb_file, cv::Mat cframe){
  ...
  if (points1.size()<1)
{
  ...
  //tracked = tracker.trackf2f(img1,img2,points,points2); //Apply LKT img1 get points1, LKT to img2 get points2 find FB_error,etc
	tracked = Latch_track(img1,img2,points,points2,lastbox); //CHANGE latch
  if (tracked){
		cv::Rect next_bb = cv::boundingRect(points2); //CHANGE
		cv::Rect init_rect;
		box_set(next_bb, init_rect, next_bb);	//CHANGE
		tbb.x = next_bb.x; tbb.y = next_bb.y; tbb.width = next_bb.width; tbb.height = next_bb.height;
      //Bounding box prediction
     //CHANGE //bbPredict(points,points2,lastbox,tbb); //Find exact location of the bounding box returned in tbb using points1 and points2
      if (/*tracker.getFB()>10 || CHANGE*/ tbb.x>img2.cols ||  tbb.y>img2.rows || tbb.br().x < 1 || tbb.br().y <1) // Failure detection using few parameters
	{
      ...
      bb.height = min(min(img2.rows-tbb.y,tbb.height),min(tbb.height,tbb.br().y));
		std::cout<<"HELLOOOOOOOO\tbb : "<<bb.x<<" "<<bb.y<<" "<<bb.width<<" "<<bb.height<<endl;
...
void TLD::detect(const cv::Mat& frame){
  ...
  GaussianBlur(frame,img,Size(9,9),1.5);
  int numtrees = classifier.getNumStructs(); //10
  float fern_th = classifier.getFernTh(); //0.6
  vector <int> ferns(10);
  float conf;
  int a=0;
  Mat patch;
  for (int i=0;i<grid.size();i++){//FIXME: BottleNeck
      if (getVar(grid[i],iisum,iisqsum)>=var){
          ...
          tmp.conf[i]=conf;
          tmp.patt[i]=ferns;
          if (conf>numtrees*fern_th) //10*0.6
	{
              dt.bb.push_back(i);
          }
      }
...
void TLD::buildGrid(const cv::Mat& img, const cv::Rect& box){
  ...
  for (int s=0;s<21;s++){
    ...
    if (min_bb_side < min_win || width > img.cols || height > img.rows)  
      continue; //bounding box is too small or bigger than image itself then continue
    scale.width = width;
    scale.height = height;
...
void TLD::clusterConf(const vector<BoundingBox>& dbb,const vector<float>& dconf,vector<BoundingBox>& cbb,vector<float>& cconf){
  ...
  for (int i=0;i<c;i++){
      ...
      if (N>0) // Take average of all the partitioned bounding boxes
	{
          cconf[i]=cnf/N;
          bx.x=cvRound(mx/N);
          bx.y=cvRound(my/N);
