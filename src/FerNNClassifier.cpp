...
void FerNNClassifier::read(const FileNode& file){
  ...
  nstructs = (int)file["num_trees"];  // nstructs = 10
  structSize = (int)file["num_features"]; //structSize = 13
...
float FerNNClassifier::measure_forest(vector<int> fern) {
  float votes = 0;
  for (int i = 0; i < nstructs; i++) {
      votes += posteriors[i][fern[i]]; //in above funtion we get leaf as a number, here we just add these numbers in every scale and its particular fern(fern has leaf) as votes.
  }
  return votes;
}
...
void FerNNClassifier::trainF(const vector<std::pair<vector<int>,int> >& ferns,int resample){
  ...
  thrP = thr_fern*nstructs;          //0.6*10 = 6                                                // int step = numX / 10;
  //for (int j = 0; j < resample; j++) {                      // for (int j = 0; j < bootstrap; j++) {
      for (int i = 0; i < ferns.size(); i++){               //   for (int i = 0; i < step; i++) {
          ...
          if(ferns[i].second==1){                           //       if  the element is a PX element  or (Y[I] == 1) {
...
void FerNNClassifier::trainNN(const vector<cv::Mat>& nn_examples){
  ...
  {                          //  For each example
      ...
      if (y[i]==1 && conf<=thr_nn) //the confidence here is negative...less means positively oriented.
      {     
