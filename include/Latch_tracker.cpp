...
float median_latch(std::vector<float> );
std::vector<float> normCrossCorrelation(const Mat& ,const Mat& , vector<Point2f>& , vector<Point2f>& );
bool Latch_track(const cv::Mat& ,const cv::Mat& , std::vector<cv::Point2f>& ,  std::vector<cv::Point2f>& ,const cv::Rect );
void box_set(const cv::Rect , cv::Rect , cv::Rect&);
...
void box_set(const cv::Rect box1,cv::Rect initb, cv::Rect &box2)
{
	initb.width = 62; initb.height =70; //FIXED Bounding BOX width to be 50 x 50
...
bool Latch_track(const cv::Mat& frame1,const cv::Mat& frame2, std::vector<cv::Point2f>& dummy_points,  std::vector<cv::Point2f>& keypoints_2,const cv::Rect next_bb)
{
...
float simmed = median_latch(similarity);
int k=0;
...
float median_latch(std::vector<float> scores)
{
